...
import { useUpdateProduct } from "@/lib/actions/product.action";
...
export default function EditProductModal({ product }) {
  ...
  const [blobUrl, setBlobUrl] = useState(product?.cover_big || null);
  const [selectedCategoryIds, setSelectedCategoryIds] = useState(product?.categoryIds || []);
  ...
  const onSubmit = useCallback(async (data) => {
    // Pass the product id, image file and selected category IDs along with form data
    updateProduct({ 
      id: product.id,
      ...data, 
      imageFile, 
      categoryIds: selectedCategoryIds,
      currentImageUrl: product?.cover_big 
    });
    close();
  }, [product.id, imageFile, selectedCategoryIds, updateProduct, close, product?.cover_big]);
