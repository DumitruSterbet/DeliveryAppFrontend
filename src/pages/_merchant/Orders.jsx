    ...
    <div className="bg-card rounded-xl border border-divider/30 p-6 hover:border-primary/20 transition-all">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-lg bg-primary/10 flex items-center justify-center">
            <Icon name="BsReceipt" size={20} className="text-primary" />
          </div>
          <div>
      ...
      {order.customerInfo && (
        <div className="mb-4 p-4 bg-neutralBg/30 rounded-lg">
          <h4 className="font-medium text-onNeutralBg mb-2">Customer Information</h4>
          <div className="text-sm text-secondary">
            <p>{order.customerInfo.name || order.customerInfo.email}</p>
            ...
            {order.items.map((item, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-neutralBg/30 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h5 className="font-medium text-onNeutralBg text-sm">{item.name || item.productName || 'Unnamed Product'}</h5>
...
export default function MerchantOrders() {
  const { data: ordersData, isLoading, isError, refetch } = useFetchMerchantOrders();
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="flex items-center gap-3">
          ...
          <span className="text-secondary">Loading orders...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-secondary">Manage and track your store orders</p>
        </div>
        <Button
          variant="outlined"
          onClick={refetch}
      ...
      {orders.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12">
          <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center mb-6">
            <Icon name="BsReceipt" size={28} className="text-primary" />
          </div>
